...
router.get("/me", async (req, res) => {
    ...
    try {
       ...
       ORDER BY level ASC, semester_number ASC`, [userId]);
        const semestersWithGPAResult = await db_1.default.query(`SELECT
         s.id,
         s.name,
         s.level,
         s.semester_number,
       ...
       ORDER BY s.level ASC, s.semester_number ASC`, [userId, userId]);
        const semestersWithGPA = semestersWithGPAResult.rows.map((sem) => ({
            ...sem,
