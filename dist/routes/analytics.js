...
const calculateGPA = (courses) => {
    const gradedCourses = courses.filter((c) => c.grade_point != null && c.grade_point !== "");
    const totalPoints = gradedCourses.reduce((sum, c) => sum + parseFloat(c.grade_point) * parseInt(c.credit_units), 0);
    const totalUnits = gradedCourses.reduce((sum, c) => sum + parseInt(c.credit_units), 0);
    return totalUnits === 0
        ? 0
        : parseFloat((totalPoints / totalUnits).toFixed(2));
};
const roundToTwo = (value) => parseFloat(value.toFixed(2));
/** Maps a CGPA to its classification label. */
const getClassificationLabel = (cgpa) => {
    if (cgpa >= 4.5)
        return "First Class";
    if (cgpa >= 3.5)
...
const getClassificationRank = (label) => {
    if (label === "First Class")
        return 4;
    if (label === "Second Class Upper")
        return 3;
    if (label === "Second Class Lower")
        return 2;
    if (label === "Third Class")
        return 1;
    return 0;
};
...
router.get("/", auth_1.default, async (req, res) => {
    try {
        // Get all semesters
        const semestersResult = await db_1.default.query("SELECT * FROM semesters WHERE user_id = $1 ORDER BY level ASC, semester_number ASC", [req.userId]);
        const semesters = semestersResult.rows;
        // Get all courses
        const coursesResult = await db_1.default.query("SELECT * FROM courses WHERE user_id = $1", [req.userId]);
        const allCourses = coursesResult.rows;
        const currentSemester = await (0, semester_1.getCurrentSemesterForUser)(req.userId);
        const currentSemesterId = currentSemester?.id ?? null;
        // GPA per semester
        const semesterGPAs = semesters.map((sem) => {
            const semCourses = allCourses.filter((c) => c.semester_id === sem.id);
            const gradedCourses = semCourses.filter((c) => c.grade_point != null && c.grade_point !== "");
            return {
                semester_id: sem.id,
                ...
                total_units: semCourses.reduce((sum, c) => sum + parseInt(c.credit_units), 0),
        ...
        const totalUnitsEarned = allCourses
            ...
            .reduce((sum, c) => sum + parseInt(c.credit_units), 0);
...
router.post("/whatif", auth_1.default, async (req, res) => {
    try {
        ...
        for (const [index, course] of hypothetical_courses.entries()) {
            ...
            const creditUnitsValid = typeof creditUnits === "number" &&
                Number.isFinite(creditUnits) &&
                creditUnits >= 0;
            const gradePointValid = typeof gradePoint === "number" &&
...
router.get("/whatif/semesters", auth_1.default, async (req, res) => {
    try {
        const semestersResult = await db_1.default.query(`SELECT id, name, level, semester_number, academic_year
         FROM semesters
         WHERE user_id = $1
         ORDER BY level ASC, semester_number ASC`, [req.userId]);
        const currentSemester = await (0, semester_1.getCurrentSemesterForUser)(req.userId);
        const currentSemesterId = currentSemester?.id ?? null;
...
router.get("/whatif/simulate-past", auth_1.default, async (req, res) => {
    ...
    try {
        ...
        const courseResult = await db_1.default.query(`SELECT c.id, c.course_code, c.course_title, c.credit_units, c.grade_point, c.grade, c.semester_id
         FROM courses c
         WHERE c.id = $1 AND c.user_id = $2`, [courseId, req.userId]);
        ...
        const allGradedResult = await db_1.default.query(`SELECT id, credit_units, grade_point
         FROM courses
         WHERE user_id = $1
           AND grade IS NOT NULL
           AND grade <> ''
           AND grade_point IS NOT NULL`, [req.userId]);
        const allGraded = allGradedResult.rows;
        // Compute actual total quality points and total completed units
        const actualTotalPoints = allGraded.reduce((sum, c) => sum + parseFloat(c.grade_point) * parseInt(c.credit_units), 0);
        const totalCompletedUnits = allGraded.reduce((sum, c) => sum + parseInt(c.credit_units), 0);
        ...
        const simulatedTotalPoints = actualTotalPoints - originalContribution + hypotheticalContribution;
...
router.get("/whatif/current-semester", auth_1.default, async (req, res) => {
    try {
        const currentSemester = await (0, semester_1.getCurrentSemesterForUser)(req.userId);
        if (!currentSemester) {
            return res.json({
        ...
        const currentCoursesResult = await db_1.default.query(`SELECT id, course_code, course_title, credit_units, grade, grade_point
         FROM courses
         WHERE user_id = $1 AND semester_id = $2
         ORDER BY created_at ASC`, [req.userId, currentSemester.id]);
        // Graded courses from completed (non-current) semesters only
        const completedGradedResult = await db_1.default.query(`SELECT credit_units, grade_point
         FROM courses
         WHERE user_id = $1
           AND semester_id <> $2
           AND grade IS NOT NULL
           AND grade <> ''
           AND grade_point IS NOT NULL`, [req.userId, currentSemester.id]);
        const completedGraded = completedGradedResult.rows;
        const previousTotalPoints = completedGraded.reduce((sum, c) => sum + parseFloat(c.grade_point) * parseInt(c.credit_units), 0);
        const completedUnits = completedGraded.reduce((sum, c) => sum + parseInt(c.credit_units), 0);
        const currentCGPA = completedUnits === 0 ? 0 : roundToTwo(previousTotalPoints / completedUnits);
        res.json({
            currentSemester: {
                id: currentSemester.id,
                name: currentSemester.name,
            ...
            courses: currentCoursesResult.rows.map((c) => ({
                id: c.id,
...
router.get("/whatif/semester-courses/:semesterId", auth_1.default, async (req, res) => {
    const { semesterId } = req.params;
    try {
        // Verify semester belongs to user
        const semResult = await db_1.default.query("SELECT id FROM semesters WHERE id = $1 AND user_id = $2", [semesterId, req.userId]);
        if (semResult.rows.length === 0) {
            return res.status(404).json({ message: "Semester not found" });
        }
        const coursesResult = await db_1.default.query(`SELECT id, course_code, course_title, credit_units, grade, grade_point
         FROM courses
         WHERE semester_id = $1 AND user_id = $2
           AND grade IS NOT NULL AND grade <> '' AND grade_point IS NOT NULL
         ORDER BY created_at ASC`, [semesterId, req.userId]);
        res.json({
...
router.get("/graduation-target", auth_1.default, async (req, res) => {
    ...
    try {
        const latestSemester = await (0, semester_1.getCurrentSemesterForUser)(req.userId);
        const completedCoursesResult = latestSemester
            ? await db_1.default.query(`SELECT * FROM courses
             WHERE user_id = $1
               AND semester_id <> $2
               AND grade IS NOT NULL
               AND grade <> ''
               AND grade_point IS NOT NULL`, [req.userId, latestSemester.id])
            : await db_1.default.query(`SELECT * FROM courses
             WHERE user_id = $1
               AND grade IS NOT NULL
               AND grade <> ''
        ...
        const finalSemesterCoursesResult = latestSemester
            ? await db_1.default.query(`SELECT id, course_code, course_title, credit_units
             FROM courses
             WHERE user_id = $1
               AND semester_id = $2
               AND (grade IS NULL OR grade = '' OR grade_point IS NULL)
             ORDER BY created_at ASC`, [req.userId, latestSemester.id])
            : { rows: [] };
        const finalSemesterCourses = finalSemesterCoursesResult.rows;
        const totalQualityPoints = completedCourses.reduce((sum, course) => sum + parseFloat(course.grade_point) * parseInt(course.credit_units), 0);
        const totalCreditUnits = completedCourses.reduce((sum, course) => sum + parseInt(course.credit_units), 0);
        const finalSemesterCreditUnits = finalSemesterCourses.reduce((sum, course) => sum + parseInt(course.credit_units), 0);
        const currentCGPA = totalCreditUnits === 0
            ? 0
            : roundToTwo(totalQualityPoints / totalCreditUnits);
        const totalGraduationUnits = totalCreditUnits + finalSemesterCreditUnits;
        const requiredQualityPoints = targetCGPA * totalGraduationUnits - totalQualityPoints;
        // rawRequired is the un-clamped GP needed; may be negative (target already secured)
        // or > 5 (mathematically impossible).
        const rawRequired = finalSemesterCreditUnits === 0
        ...
        const requiredAverageGP = rawRequired === null
            ? null
            : Math.max(1.0, rawRequired);
        const maxAchievableCGPA = totalGraduationUnits === 0
            ? 0
            : roundToTwo((totalQualityPoints + finalSemesterCreditUnits * 5) /
        ...
        const minAchievableCGPA = totalGraduationUnits === 0
            ? 0
            : roundToTwo((totalQualityPoints + finalSemesterCreditUnits * 1.0) /
                totalGraduationUnits);
        ...
        res.json({
            ...
            requiredAverageGP: status === "NOT_ACHIEVABLE"
                ? null
